...
RSpec.describe "DELETE /api/session", :db, type: :request do
  init_request_spec
  ...
  it "is redirect (303 see other)" do
    delete "/api/session", params.to_json, request_headers
  ...
  it "is unprocessable on logout with wrong username params" do
    with_session(:logout) do
      delete "/api/session", {**params, username: "a"}.to_json, request_headers
  ...
  it "is unprocessable on logout with wrong password" do
    with_session(:logout) do
      delete "/api/session", {**params, password: "a"}.to_json, request_headers
      expect(last_response).to be_unprocessable
      expect(last_response.headers["Content-Type"]).to eq "application/json; charset=utf-8"
      json = JSON.parse(last_response.body, symbolize_names: true)
      expect(json).to eq({
  ...
  it "is unprocessable on logout with wrong username and password" do
    with_session(:logout) do
      delete "/api/session", {**params, username: "a", password: "a"}.to_json, request_headers
      expect(last_response).to be_unprocessable
      expect(last_response.headers["Content-Type"]).to eq "application/json; charset=utf-8"
      json = JSON.parse(last_response.body, symbolize_names: true)
      expect(json).to eq({
